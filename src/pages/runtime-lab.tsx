...
export default function RuntimeLabPage() {
  ...
  async function run(action: string, payload: Record<string, unknown> = {}) {
    setStatus('loading');
    try {
      const response = await fetch('/api/runtime-lab', {
        method: 'POST',
            ...
            {runtime_lab.cards.map((card, index) => {
              ...
              return (
                <article key={card.title} className="rounded-lg border border-slate-700 bg-slate-900 p-5 shadow-sm">
                  ...
                  {index === 1 && <label className="block text-xs text-slate-300">{runtime_lab.cacheLabel}<input value={cacheKey} onChange={(event) => setCacheKey(event.target.value)} placeholder={runtime_lab.cachePlaceholder} disabled={status === 'loading'} className="mt-2 w-full rounded border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-white placeholder:text-slate-600 focus:border-emerald-500 focus:outline-none" /></label>}
                  {index === 3 && <label className="block text-xs text-slate-300">{runtime_lab.primeLabel}<input inputMode="numeric" value={primeCount} onChange={(event) => setPrimeCount(event.target.value)} placeholder={runtime_lab.primePlaceholder} disabled={status === 'loading'} className="mt-2 w-full rounded border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-white placeholder:text-slate-600 focus:border-emerald-500 focus:outline-none" /></label>}
                  <button type="button" disabled={status === 'loading'} onClick={() => run(index === 0 ? 'environment' : index === 1 ? 'cache' : index === 2 ? 'system' : 'primes', index === 1 ? { key: cacheKey } : index === 3 ? { count: primeCount } : {})} className="mt-5 rounded bg-emerald-500 px-3 py-2 font-sans text-sm font-medium text-slate-950 transition-colors hover:bg-emerald-400 disabled:cursor-not-allowed disabled:bg-slate-700 disabled:text-slate-400">{status === 'loading' ? 'Running…' : card.action}</button>
                </article>
              );
            })}
          </div>
